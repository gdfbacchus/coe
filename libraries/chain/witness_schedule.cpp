...
void update_witness_schedule4( database& db )
{
   ...
   if( db.has_hardfork( STEEMIT_HARDFORK_0_5__54 ) )
   {
      ...
      for( uint32_t i = 0; i < wso.num_scheduled_witnesses; i++ )
      {
         ...
         std::cout << __PRETTY_FUNCTION__
         << " witness: " << std::string(wso.current_shuffled_witnesses[ i ])
         << " version: " << fc::string(witness.running_version)
         << " hfvv: " << fc::string(witness.hardfork_version_vote)
         << " hftv: " << fc::string(witness.hardfork_time_vote) << '\n';
...
void update_witness_schedule(database& db)
{
   if( (db.head_block_num() % STEEMIT_MAX_WITNESSES) == 0 ) //wso.next_shuffle_block_num )
   {
      ...
      if( props.num_pow_witnesses == 0 || db.head_block_num() > STEEMIT_START_MINER_VOTING_BLOCK )
      {
         ...
         const auto& schedule_idx = db.get_index<witness_index>().indices().get<by_schedule_time>();
         std::cout << __PRETTY_FUNCTION__ << " sched_siz: " << schedule_idx.size() << '\n';
         auto sitr = schedule_idx.begin();
      ...
      const auto& pow_idx = db.get_index<witness_index>().indices().get<by_pow>();
      std::cout << __PRETTY_FUNCTION__ << " pow_siz: " << pow_idx.size() << '\n';
      ...
      std::cout << __PRETTY_FUNCTION__ << " wit_siz: " << active_witnesses.size() << '\n';
